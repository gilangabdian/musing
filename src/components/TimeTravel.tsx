...
export default function TimeTravel() {
  ...
  const animate = (time: number) => {
    ...
    if (glitch && !isGlitchingRef.current) {
      ...
      // SNAP to integer so it freezes perfectly centered!
    ...
    if (glitch) {
      ...
      if (timeToGlitch > 0 && timeToGlitch < 1.0) {
        ...
        currentSpeed *= 0.2 + 0.8 * factor;
      }

      if (Math.abs(distance) < currentSpeed * 2) {
        // Glide smoothly into place (easing)
    ...
    <div
      ...
      {!isStarted ? (
        <form onSubmit={handleStart} className="z-10 flex flex-col items-center gap-6 w-full max-w-md px-6">
          <label className="text-zinc-400 text-lg md:text-xl tracking-widest font-light text-center">
            <span className="uppercase">To what year will you travel?</span>{" "}
            <span className="text-xs">(maybe back to your childhood?)</span>
          </label>
          <input
            type="number"
          ...
          <button
            type="submit"
            className="mt-6 px-8 py-3 text-zinc-400 bg-zinc-800 hover:bg-zinc-900 transition-colors duration-200 text-sm tracking-widest uppercase">
            Go
          </button>
        </form>
      ) : (
        <div className="z-10 flex flex-col items-center justify-center gap-8">
          ...
          <button
            onClick={handleReset}
            className={`text-zinc-500 hover:text-white transition-all duration-500 text-lg tracking-widest uppercase ${
